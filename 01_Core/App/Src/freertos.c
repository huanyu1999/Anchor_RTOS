...
const osSemaphoreAttr_t uwbIntSem_attr = { .name = "uwbIntSem", .cb_mem = &uwbIntSemCB, .cb_size = sizeof(StaticSemaphore_t) };

osSemaphoreId_t w5500IntSem;
StaticSemaphore_t w5500IntSemCB;
const osSemaphoreAttr_t w5500IntSem_attr = { .name = "w5500IntSem", .cb_mem = &w5500IntSemCB, .cb_size = sizeof(StaticSemaphore_t) };
...
uint32_t  task9_buffer[256];
osStaticThreadDef_t task9CB;
...
void task8_canReceiveHandle(void *arg);
...
void MX_FREERTOS_Init(void) {
    ...
    task8_Handle                   = osThreadNew(task8_canReceiveHandle, NULL, &task8_attr);
    task9_Handle                   = osThreadNew(task_eth, NULL, &task9_attr);
    task10_emmcHandle              = osThreadNew(task10_emmcTest, NULL, &task10_attr);
...
void task2_voiceOut(void *arg)
{
    ...
    for (;;) {
        ...
        dev_jq8400ConvertDis((float)voiceOutputDis / 1000.0); // 在此处直接输出最终的距离，后续调试看情况是否需要加上互斥量保护
        send(0, test, ARRAY_LENGTH(test));
        tick += 1100;
        osDelayUntil(tick);
    }
}
...
void task3_canSend(void *arg) {   
    uint32_t CanExtId4Button = 0xAAA3;
    for (;;) {
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin) {
    if (GPIO_Pin == Dw1000_RSTn_Pin) {
        ...
        osSemaphoreRelease(uwbIntSem);    // 在这里释放信号量
