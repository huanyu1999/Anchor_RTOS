...
bool heap_insert(min_heap* heap,  uint8_t original_index, int32_t value)
{
    ...
    heap->nodes[cur_index].index = original_index;
...
// bool heap_peek_min(min_heap* heap, int32_t *value, uint8_t *index) 
...
static void swap_nodes(min_heap *heap, int pos1, int pos2)
{
    ...
    heap->pos_map[heap->nodes[pos1].index] = pos2;
    heap->pos_map[heap->nodes[pos2].index] = pos1;
