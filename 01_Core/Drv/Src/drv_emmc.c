...
uint8_t drv_emmcInit(void)
{
    ...
    emmc_handle.Init.ClockDiv = SDIO_TRANSFER_CLK_DIV;                            // 初始化时钟

    if(HAL_MMC_Init(&emmc_handle) != HAL_OK)
    if(HAL_MMC_InitCard(&emmc_handle) != HAL_OK)
    {
...
void HAL_MMC_MspInit(MMC_HandleTypeDef *hmmc)
{
    ...
    GPIO_InitStructure.Pin = EMMC_CLK_PIN | EMMC_D0_PIN | EMMC_D1_PIN | EMMC_D2_PIN | EMMC_D3_PIN | EMMC_D4_PIN | EMMC_D5_PIN | EMMC_D6_PIN | EMMC_D7_PIN;
...
{
    if(HAL_MMC_WriteBlocks_DMA(&emmc_handle, (uint8_t *)pData, readAddr, 
        numOfBlocks) != HAL_OK)
    {
        return EMMC_ERROR;
    }
...
{
    if(HAL_MMC_WriteBlocks_DMA(&emmc_handle, (uint8_t *)pData, writeAddr, 
        numOfBlocks) != HAL_OK)
    {
        return EMMC_ERROR;
    }
