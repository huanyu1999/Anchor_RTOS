...
static void twrAnchor_rxErrorOrTimeout()
{
    dwDevice_t* dev = get_the_local_structure_of_dev();
    ...
    if (dev->wait4final == WAIT4TAGFINAL)       // 说明是接收final帧出现异常
    {
        dev->wait4final = 0; 
        anch_rxRenableImmdiate();
    }
    else                                        // 说明是接收poll帧或者resp帧出现异常
    {
...
static void anch_txRespOrRxReenale(void)
{
    ...
    else      // 继续接收
    {
        if (dev->remainingRespToRx == 0) // 不再期望接收任何Resp帧，接收final
        {
            ...
            dwt_setpreambledetecttimeout(PRE_TIMEOUT);     // 设置接收前导码超时时间
            int ret = dwt_rxenable(DWT_START_RX_DELAYED);  // 延时开启接收机
            if(ret == DWT_ERROR)                           // 打开失败，立即重新打开接收，相当于本次测距失败，重新接收poll
            {
                anch_rxRenableImmdiate();
            }
        }
        else if (dev->remainingRespToRx == -1)              // 该值还为初始值，说明poll帧接收异常，处理开启立即接收
        {
            anch_rxRenableImmdiate();
        }
